...
class MM_nest(nn.Module):
    ...
    def forward(self, meta_cat, meta_con, cli_x, der_x):
        ...
        cli_features = self.cli_nest(cli_x)
        der_features = self.der_nest(der_x)

        combined_x = torch.cat((cli_features, der_features), dim=1) 
        x = self.feature_reduce(combined_x)
...
def main():
    ...
    model = MM_nest(num_classes=5, hidden_dim=128).to(device)
    print(model)
