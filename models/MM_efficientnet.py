...
class efficientnet_MMC(nn.Module):
    def __init__(self, num_classes=5, hidden_dim=128, model_name='', num_heads=2, dropout_rate=0.5, meta_dim=32):
        super(efficientnet_MMC, self).__init__()

        # Backbone pretrained CNN models for clinical and dermoscopy branches
        self.cli_cnn = timm.create_model('efficientnet_b4', pretrained=True)
        self.der_cnn = timm.create_model('efficientnet_b4', pretrained=True)
        ...
        self.feature_reduce = nn.Sequential(
            nn.Linear(2128, hidden_dim*2),  # Adjust input size to match concatenated features
            nn.ReLU(inplace=True),
...
def main():
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = efficientnet_MMC(num_classes=5, hidden_dim=128).to(device)
    print(model)
