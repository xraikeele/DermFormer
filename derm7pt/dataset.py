...
class Derm7PtDataset(object):
    ...
    def __init__(self, dir_images, metadata_df, train_indexes, valid_indexes, test_indexes, crop_amount=25):
        ...
        # Selecting only columns of interest without one-hot encoding
        self.meta_train = self.df.loc[train_indexes, ['elevation', 'sex', 'location', 'level_of_diagnostic_difficulty', 'management']]
        self.meta_valid = self.df.loc[valid_indexes, ['elevation', 'sex', 'location', 'level_of_diagnostic_difficulty', 'management']]
        self.meta_test = self.df.loc[test_indexes, ['elevation', 'sex', 'location', 'level_of_diagnostic_difficulty', 'management']]

    #def set_df_numeric_labels(self):
    #    """Add numeric values to the columns in the df."""
        # You may have additional logic here to convert strings to numeric values as needed.
    #    pass

    def check_myself(self):
        """Check the properties of the class match the columns in self.df"""
        # Ensure the class properties match the expected columns in self.df
        pass

    def get_dict_labels(self, df_column_names):
        """Return a dictionary mapping unique values in df_column_names to numeric labels."""
